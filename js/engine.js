...
var Engine = (function(global) {
    ...
    function main() {
        ...
        if ((Date.now() - lastTimeCreate) / 1000.0 > 1.5) {
            var e = new Enemy();
            allEnemies.push(e);
            lastTimeCreate = Date.now();
        }
    ...
    function updateEntities(dt) {
        ...
        //回收走到头的敌人
        allEnemies = allEnemies.filter(function(_this) {return _this.x < 500;});
        player.update();
    }
    function checkCollisions() {
        //碰撞
        allEnemies.forEach(function(enemy, i) {
            if (enemy.y == player.y && enemy.x > ( -80 + (player.x + 0) * canvas.width / 5) && enemy.x < ( -120 + (player.x + 2) * canvas.width / 5) ) {
                player = new Player();
            }
        });
    }
